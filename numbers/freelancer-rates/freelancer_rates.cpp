...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
   double dailied_rate = daily_rate(hourly_rate);
    double discounted_rate = apply_discount(dailied_rate, discount);
    return static_cast<int>(std::ceil(discounted_rate * 22));
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double dailied_rate = daily_rate(hourly_rate);
    double discounted_rate = apply_discount(dailied_rate, discount);
    return static_cast<int>(budget / discounted_rate);
}
